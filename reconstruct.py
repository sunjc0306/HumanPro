...
class ReMesh:
    ...
    def stich_case(self,vertices, faces,color_pils, normal_pils,econdata):
        # masks, target_colors, target_normals = self.preprocess(clr_img, nrm_img)
        kernal = torch.ones(3, 3)
        erode_iters = 2
        normals = []
        ...
        resolution = 512
        for normal, color in zip(normal_pils, color_pils):
            normal = normal.resize((resolution, resolution), Image.BILINEAR)
            normal = np.array(normal).astype(np.float32) / 255. * 2 -1
            mask = normal[..., 3:]  # alpha
            mask_troch = torch.from_numpy(mask).unsqueeze(0)
            for _ in range(erode_iters):
                mask_torch = kornia.morphology.erosion(mask_troch, kernal)
            ...
            color = color.resize((resolution, resolution), Image.BILINEAR)
            color = np.array(color).astype(np.float32) / 255.
            color_mask = color[..., 3:]  # alpha
            # color_dilate = color[..., :3] * color_mask  + bg_color * (1 - color_mask)
            color_dilate = color[..., :3] * mask_erode + bg_color * (1 - mask_erode)
        ...
        case_path = './debug'
        mesh_remeshed = trimesh.Trimesh(vertices=vertices.detach().cpu().numpy(), faces=faces.detach().cpu().numpy())
        mesh_remeshed.export(f'{case_path}/{case}_smplx.obj')
        depth_F, depth_B = econdata.render_depth(vertices[None], faces[None])
        depth = self.renderer.render_depth(vertices, faces)
        os.makedirs(case_path, exist_ok=True)
        from lib.common.BNI_utils import tensor2arr,depth2arr,save_normal_tensor
        from lib.common.BNI import BNI
        for idx in range(depth.shape[0]//2):
            normal_F_arr = target_normals[idx:idx + 1].squeeze(0).detach().cpu().numpy()
            normal_B_arr = (target_normals[(idx+3):(idx+3) + 1]).squeeze(0).detach().cpu().numpy()
            # mask_normal_arr = tensor2arr(masks[idx:idx + 1], True)
            mask_normal_arr = masks[idx:idx + 1,:,:,0].squeeze(0).detach().cpu().numpy()
            # depth_F_arr = depth[idx,:,:,0].float().detach().cpu().numpy()
            # depth_B_arr = (depth[idx + 3,:,:,0]).float().detach().cpu().numpy()
            # mask_depth_arr = depth[idx, :, :, 1].detach().cpu().numpy()
            #
            ...
            BNI_dict = {}
            depth_F_arr = depth2arr(depth_F)
            depth_B_arr = depth2arr(depth_B)
            BNI_dict["normal_F"] = normal_F_arr
            BNI_dict["normal_B"] = normal_B_arr[:,::-1,:]
            ...
            # BNI_dict["mask"] = mask_normal_arr > 0.
            # BNI_dict["depth_F"] = depth_F_arr
            # BNI_dict["depth_B"] = - depth_B_arr
            # BNI_dict["depth_mask"] = mask_depth_arr > 0.
            # BNI process
            BNI_dict_1 = torch.load('./debug/BNI_dict.pt')
            BNI_dict['normal_F'] = BNI_dict_1['normal_F']
            BNI_dict['normal_B'] = BNI_dict_1['normal_B']
            BNI_dict['mask'] = BNI_dict_1['mask']
            BNI_dict['depth_F'] = BNI_dict_1['depth_F']
            BNI_dict['depth_B'] = BNI_dict_1['depth_B']
            BNI_dict['depth_mask'] = BNI_dict_1['depth_mask']
            BNI_object = BNI(
                dir_path=case_path,
                name='0',
                BNI_dict=BNI_dict,
            ...
            BNI_object.F_B_trimesh.export(f'{case_path}/{case}_F_B_trimesh.obj')
        ...
        colors = meshes.textures.verts_features_packed().float()
        save_mesh(f'./{case_path}/result_clr_scale{self.opt.scale}_{case}.obj', vertices, faces, colors)
        self.evaluate(vertices, colors, faces, save_path=f'{case_path}/result_clr_scale{self.opt.scale}_{case}.mp4',
                      save_nrm=True)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--config",  help="path to the yaml configs file", default='configs/inference-768-6view.yaml')
