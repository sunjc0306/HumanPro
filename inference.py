            ...
            if cfg.save_mode == 'concat': ## save concatenated color and normal---------------------
                ...
                for i in range(bsz//num_views):
                    ...
                    for j in range(num_views):
                        ...
                        os.makedirs(os.path.join(carving.opt.res_path, scene), exist_ok=True)
                        save_image(normal, os.path.join(carving.opt.res_path, scene, normal_filename))
                        save_image(color, os.path.join(carving.opt.res_path, scene, rgb_filename))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/inference-768-6view.yaml')
