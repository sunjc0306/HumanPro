...
class NormalsRenderer:
    ...
    def render_depth(self,
        ...
        vertices_clip = vert_hom @ self._mvp.transpose(-2,-1) #C,V,4
        rast_out,_ = dr.rasterize(self._glctx, vertices_clip, faces, resolution=self._image_size, grad_db=False) #C,H,W,4
        vert_depth = vertices[:,2:].contiguous()
        depth, _ = dr.interpolate(vert_depth, rast_out, faces) #C,H,W,3
        alpha = torch.clamp(rast_out[..., -1:], max=1) #C,H,W,1
        depth = torch.concat((depth,alpha),dim=-1) #C,H,W,4
        depth = dr.antialias(depth, rast_out, vertices_clip, faces) #C,H,W,4
        if return_triangles:
            return depth, rast_out[..., -1]
