...
class StableUnCLIPImg2ImgPipeline(DiffusionPipeline):
    ...
    def _encode_image(
        ...
        def vae_encode(tensor):
            ...
            image_latents = []
            for i in range(image_pt.shape[0]):
                image_latents_item = self.vae.encode(image_pt[i][None]).latent_dist.mode() * self.vae.config.scaling_factor
                image_latents.append(image_latents_item)
        ...
        if not output_type == "latent":
            ...
            with torch.no_grad():
                ...
                for i in range(latents.shape[0]):
                    image_item = self.vae.decode(latents[i][None] / self.vae.config.scaling_factor, return_dict=False)[0]
